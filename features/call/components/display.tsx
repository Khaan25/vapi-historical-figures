  ...
  useEffect(() => {
    const onMessageUpdate = (message: Message) => {
      ...
      if (
        ...
        message.toolCalls[0].function.name === "suggestShows"
      ) {
        setStatus("show");
        vapi.send({
        ...
        message.type === MessageTypeEnum.TOOL_CALLS &&
        (message.toolCalls[0].name === "confirmDetails" ||
          message.toolCalls[0].name === "bookTickets")
      ) {
        ...
        setStatus(
          message.toolCalls[0].name === "confirmDetails" ? "confirm" : "ticket"
        );
      }
    };
